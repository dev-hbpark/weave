// MANUAL byo-ssh style-capture harness (NOT CI). Run against the prod preview
// (HMR-immune) + byo-ssh server. STYLES env picks the styles. Shots → /tmp/style-shots.
...
const diag = (m: string) => appendFileSync("/tmp/pw-style-diag.log", `${m}\n`);
...
for (const style of STYLES) {
  test(`generate + capture: ${style}`, async ({ page }) => {
    ...
    try {
      ...
      diag(`[${style}] DONE items=${await deepCount(page)}`);
    }
  });
}
