  ...
  layoutKinds: [
    ...
    },
    {
      ...
      description: [
        ...
        "SIZE STACKED REGIONS (the #1 flex composition bug): when you stack regions in a flex COLUMN (e.g. a title band, then a table, then a footer) the DEFAULT child policy (grow 0, basis 'auto') gives EACH child its own frame size — so a region whose frame you made tall, or that holds a tall grid, eats a huge slice and the rest is cramped (a ONE-ROW header ballooning to ~40% of the slide is exactly this). You MUST decide each region's share EXPLICITLY: the small band/header gets grow 0 + a small explicit basis (or make that band a HUG-height frame via weave.frame.setSizing height:'hug' so it shrinks to its single row), and the MAIN region (the table/body) gets grow 1 so it absorbs the remaining height. Never leave sibling regions on the default policy and hope — set them.",
    ...
    },
    {
      ...
      description: [
        ...
        "SHAPE THE GRID FOR YOUR DATA (the #1 grid mistake): set `columns` to the table's LOGICAL column count up-front — a 3-field comparison = 3 fr tracks, a 4-up card grid = 4, a weekday calendar = 7. NEVER use `columns:[]` for a table/card grid: an empty array is ONE full-width track, so every item lands in a single column and stacks into a 1-row strip. You do NOT need to pre-declare every row — give the columns you want plus one `rows` track and the grid GROWS rows as you add items (and to cover any explicit `{row}` you place). So: pick the column count = your data's width, then fill left-to-right, top-to-bottom (omit per-child column/row to auto-flow, or set them 1-based and consistent with that column count). Place N data cells across C columns ⇒ rows = ceil(N / C); keep the column count fixed so headers stay aligned.",
        "HEADER + BODY = ONE GRID (do NOT split them): a table's header row and its data rows belong to the SAME auto-grid — put the header cells in ROW 1 and the data in rows 2..N, all sharing the grid's columns. This makes the header columns line up with the body columns AUTOMATICALLY and keeps the header exactly one row tall. Do NOT build a SEPARATE header grid and a SEPARATE body grid stacked in a flex: the two grids have independent column tracks (so they won't line up) AND you'd then have to hand-size the flex split or the one-row header balloons to half the slide. If the header needs a different height than the body rows, give the grid a smaller FIRST ROW track (e.g. rows = [ a shorter header track, then equal body tracks ]) — still ONE grid. Only split into separate frames when the regions are genuinely different layouts (not a header over its own columns).",
...
export const WEAVE_TASK_PRIMER = [
  ...
  "- INTERPRET DATA, DON'T TRANSCRIBE IT (top priority, equal to MOOD — the SINGLE most common regression): tabular / CSV / list / numeric input is SOURCE MATERIAL, NOT a layout instruction. Re-emitting a table as a grid of cells is a DEFECT unless the user EXPLICITLY asked for a table or genuinely needs row-by-row lookup. FIRST decide what the data MEANS, then build the form that makes that meaning LAND: time / trend → line or area chart; comparison across categories → bar chart; part-of-whole / proportion → pie or stacked; distribution / correlation → scatter / bubble; flow or hierarchy → sankey / treemap; ONE decisive figure → a BIG number + caption (never a one-row table); a relationship (trade-off, overlap, nested scope, process / cycle) → a shape-built diagram (2×2 / SWOT, Venn, concentric circles, funnel, timeline with arrows). Use weave.chart.add for any real dataset. Show only the few figures that carry the message — keep the headline / top-few / total / delta, OMIT or summarize the long tail ('top 5 … +N more'). If your instinct is 'just render the table', STOP and choose a chart, diagram, or big-number treatment instead. Default = TRANSFORM, not transcribe.",
  ...
  "- GROUP MULTI-ITEM CLUSTERS IN LAYOUT FRAMES — this is FIT-SAFETY, not a slide template. Whenever 2+ related content items sit together (a header's title+subtitle, a column's stack, a card, a stat row, a card grid) add a nested frame (containerId = the slide, presentable:false), give it weave.frame.setLayout (auto-flex row/column or auto-grid: direction, gap, padding, justify, align), and add those items as its CHILDREN so they never collide or overflow. That guarantees FIT — it does NOT mean every slide is the same stack of full-width bands (header→body→columns→stat-row). VARY THE MACRO COMPOSITION per slide and per deck (if the task carries a [이번 변주] 구도 line, that archetype IS the committed macro-shape — apply it): a full-bleed hero with text overlaid in a corner, an asymmetric 0.38:0.62 split, one dominating big-number, layered/overlapping cards, a sidebar shell, a diagonal flow, a full-canvas diagram, a centered stage with generous margins — choose the structure that fits THIS content, never a default grid-of-bands. A single well-placed element needs no wrapper frame; backdrops, full-bleed images and accent shapes use absolute/overlay placement freely. The only real defect is hand-placing a CLUSTER of content by absolute x/y so it drifts out of alignment — group THAT into a frame; deliberate free-form composition is welcome.",
  "- AN ACCENT BELONGS TO ITS CARD, NOT TO A FLOATING SIBLING: a card / section's coloured HEADER STRIP, top bar, or accent rule must be the card frame's OWN decoration.fill (round only the top via attrs.cornerRadii { tl, tr }) or a CHILD placed INSIDE that frame — NEVER a separate absolute shape positioned by x/y to overlap the card. A separate shape only LOOKS aligned at author time and DRIFTS the instant the card is moved, resized, or reflowed (the two live in independent coordinate spaces). Same for connector lines between cards that must track them: anchor to the layout, don't hand-place a twin.",
  "- DON'T PAD A REGION WITH WHITESPACE TO FILL IT (sparse card + small text reads as empty): when a fixed-height card / region holds only a little content, make the frame size to its content with weave.frame.setSizing height:'hug' (it becomes exactly as tall as its stack) instead of inflating gap/padding to stretch a few items across a tall box. Keep gap MODEST and the SAME across sibling cards — never give one card a large px gap and its neighbour a tiny one (inconsistent, lopsided). If a card looks empty, ADD a visual or ENLARGE the text to its region, do not spread the existing items apart.",
  "- WHEN a literal table IS the right call (lookup/reference, or the user asked): build it as an auto-grid with explicit tracks, NEVER nested flex rows. TEXT is AUTO-HEIGHT and must WRAP to its cell: BIND its WIDTH (flex COLUMN → align/alignSelf 'stretch' = the cross axis IS the width; grid → the column track) so a long line never overflows. Only the HEIGHT follows content — do NOT grow/vertically-stretch text or pin a height. Size equal regions with the FRAMES (tracks/grow), not the leaf text.",
...
export const WEAVE_DOMAIN_KNOWLEDGE = [
  ...
  "   VARY THE MACRO COMPOSITION — auto-layout frames keep GROUPS aligned and overflow-free; they are NOT a mandate",
  "   that every slide be the same stack of full-width bands (header → body → columns → stat-row). That uniform",
  "   template is the #1 cause of every deck looking alike. Choose the overall SHAPE that fits the content and let",
  "   it differ slide-to-slide and run-to-run: a full-bleed hero (background covers the slide, text overlaid in one",
  "   corner with a scrim), an ASYMMETRIC split (e.g. 0.38:0.62, one side dominating), a single dominating",
  "   BIG-NUMBER, LAYERED/overlapping cards with z-depth, a SIDEBAR shell (narrow accent rail + wide main), a",
  "   DIAGONAL flow, a full-canvas DIAGRAM (timeline/flow/Venn/2×2 owning the whole slide), or a CENTERED stage",
  "   with generous margins. Compose the GROUPS into that shape (unequal grid tracks, a frame placed off-center, an",
  "   absolutely-placed hero element over a layout body) — the per-group auto-layout still guarantees fit inside",
  "   each group. Do not default to an even grid of equal bands; pick the structure the content asks for.",
