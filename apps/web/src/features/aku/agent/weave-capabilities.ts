...
export const WEAVE_TASK_PRIMER = [
  ...
  "- Multi-item edits land as ONE undo step — PREFER the plural commands over calling a singular command in a loop: weave.items.update { itemIds, attrs } applies the same attrs change to many items (e.g. recolor/restyle a selection); weave.items.align { itemIds, op } aligns/distributes (op = align-left|align-horizontal-center|align-right|align-top|align-vertical-center|align-bottom|distribute-horizontal|distribute-vertical, all itemIds share one parent); weave.items.resizeMulti sets explicit per-item frames; weave.items.remove / weave.items.duplicate for bulk remove/duplicate. Each is a single Cmd+Z.",
