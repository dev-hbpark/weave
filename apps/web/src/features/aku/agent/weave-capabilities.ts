...
export const WEAVE_TASK_PRIMER = [
  ...
  "- THEME COLORS — DEFAULT to theme tokens because they RE-SKIN when the user switches theme, but the CONTENT'S MOOD wins over the current theme: if a specific literal color expresses the mood better, use it. Use a `var(--token)` STRING for structural/neutral roles — slide background var(--bg-page)/var(--bg-page-soft) (NOT a lazily hand-picked dark/light hex; that just makes the slide ignore the theme), text var(--text-strong|default|soft|muted), emphasis var(--accent|-strong|-soft), panels var(--surface-1|2) (translucent) or var(--bg-page-soft) (solid), a 4-hue categorical palette via var(--domain-slide|canvas|block|media-accent). Theme colors are NOT solid-only — use tokens (or literals) as GRADIENT stops too (e.g. var(--accent)→var(--accent-strong)). Tokens work in any color field (text attrs.color, decoration.fill/.stroke PaintSpec color, slide/frame background). Reach for a LITERAL color when it suits the mood better, and for brand/logo, data-bound, photographic, or status (green/amber/red) colors.",
...
export const WEAVE_DOMAIN_KNOWLEDGE = [
  ...
  "   NESTED FRAMES ARE THE PRIMARY LAYOUT TOOL: nest a frame (containerId = another frame's id), give it a",
  "   layout via weave.frame.setLayout (auto-flex row/column or auto-grid), and it becomes a self-arranging",
  "   container — header band, two-column split, card, card grid, sidebar, stat row. Compose clean aligned",
  "   layouts from nested layout frames instead of hand-placing every item.",
  ...
  "   color to express THAT. Theme tokens are your DEFAULT tool: a var(--token) string re-skins when the user",
  "   switches theme (weave ships 10 themes, each publishing the SAME semantic token set; the string is stored",
  "   verbatim and CSS resolves it per the active [data-theme]), and the active theme is usually chosen to match",
  "   the mood — so for structural / neutral roles (page background, body & heading text, generic panels, a",
  "   primary accent) PREFER tokens. BUT you are the designer: when the content's mood calls for a colour the",
  "   theme can't express (a brand palette, an evocative scheme, a signature accent or gradient), USE A LITERAL",
  "   colour — never flatten the content's mood just to keep everything theme-reactive. The mood of the document",
  "   wins over the current theme. (Only avoid the LAZY literal — a hand-picked grey background where",
  "   var(--bg-page) would do; a deliberate, mood-driven literal is correct.) Tokens AND literals both work",
  "   SOLID or GRADIENT — theme colors are NOT solid-only (see the gradient line below).",
  "",
  ...
  "     • gradient     — theme colors are NOT solid-only: use them as GRADIENT stops too (linear or radial),",
  "                      and the gradient stays theme-reactive. e.g. a hero background units:[{ kind:",
  "                      'decoration.fill', attrs:{ type:'linear-gradient', angle:135, stops:[{offset:0,",
  "                      color:'var(--accent)'},{offset:1,color:'var(--accent-strong)'}] } }], or",
  "                      var(--bg-page)→var(--bg-page-soft) for a subtle backdrop. Literal-stop gradients are",
  "                      equally fine when the mood needs a specific blend.",
  "",
