  ...
  itemKinds: [
    ...
    },
    {
      ...
      description: [
        ...
        "SIZING: size the font by RATIO — attrs.fontSizeSpec { kind:'ratio', value:0..1 } where value is a fraction of the height of the frame THIS TEXT IS A DIRECT CHILD OF (its containerId frame), NOT the slide or canvas unless the slide is its direct parent. The ratio resolves against the IMMEDIATE parent frame, so the SAME ratio is bigger in a tall frame and smaller in a short nested frame. The ratio is just a UNIT CONVERSION of a px target (ratio = target px ÷ the parent frame's px height) — NOT a cue to 'fill' the frame. SHARED-FRAME BUDGET — the #1 cause of overflow: when a frame holds MORE THAN ONE item the items SHARE its height, so a text does NOT get the whole frame. Before sizing, budget it: usable height = parent frame px height − padding(top+bottom) − the gaps between children; size each text against ITS OWN share of that budget, and the SUM of the children's rendered heights (each ≈ fontSize × lineHeight × wrapped-line-count) must fit the usable height WITH MARGIN (the whole stack + gaps ≤ ~85%). Sizing a text as if it alone fills a frame it actually shares with a title / subtitle / body + gaps is exactly what overflows. Roles (heading ~0.06–0.09, subheading ~0.04, body ~0.03 of a SLIDE-height parent) are per-text TARGETS, not licence to fill; those numbers assume the parent FILLS the slide, so inside a shorter nested frame divide the px target by THAT frame's px height. The size must FIT: the text must sit inside its frame with margin to spare — if it would crowd or overflow, size DOWN and shorten the copy, never oversize. Do NOT use a fixed px size (a bare fontSize number or { kind:'px' }). NEVER put a fraction in the plain fontSize number — a 0..1 there renders as sub-pixel text; express ratios only via fontSizeSpec { kind:'ratio' }. AND THE REVERSE: a fontSizeSpec { kind:'ratio' } value MUST be the 0..1 fraction (≈0.03–0.09), never a px magnitude — { kind:'ratio', value:24 } resolves to 24 × the parent height (~25000px, the whole canvas swallowed by one glyph). If you ever mean an absolute px size, that is the ONE valid use of { kind:'px', value }; do not smuggle a px number into a ratio.",
...
export const WEAVE_DOMAIN_KNOWLEDGE = [
  ...
  "   × the parent frame's own height ratio, chained through every level of nesting). A heading alone in a small",
  "   card needs a LARGER ratio than the same heading on a full slide, because the ratio is per-parent-height.",
  "   SHARED-FRAME BUDGET (the #1 overflow cause): items in ONE frame SHARE its height (minus padding and the gaps",
  "   between them), so a text does NOT own the whole frame — size each against ITS OWN share, and keep the SUM of",
  "   the children's rendered heights (each ≈ fontSize × lineHeight × line-count) within the usable height WITH",
  "   margin (stack + gaps ≤ ~85%). The classic mistake is sizing a text to 'fill' a frame it actually shares with a",
  "   title / body + gaps. Anchor to the px role targets (heading ~5–7% of CANVAS height, body ~3%) and treat the",
  "   ratio purely as target-px ÷ parent-frame-px-height, never as a cue to fill. Do NOT use a fixed px size: avoid a",
