...
export const WEAVE_TASK_PRIMER = [
  ...
  "- Top-level frames are SLIDES; frames MAY be nested to group items into a unit. For a plain rectangle inside a slide (panel, card, background block, divider, button), prefer a SHAPE (kind:'shape', rectangle) coloured with a decoration.fill unit; use a nested frame only to group multiple items together.",
...
export const WEAVE_DOMAIN_KNOWLEDGE = [
  ...
  "   Frames MAY be nested (containerId = another frame's id) to GROUP items into one movable/clippable unit.",
  "   For a plain rectangle inside a slide — panel, card, background block, divider, button shape — PREFER a",
  "   SHAPE (kind:'shape', rectangle), not a frame; use a nested frame only to group several items together.",
  "   A nested frame is itself a slide by default unless the user excludes it from the deck.",
  "",
  ...
  "",
  "4) THEME COLORS — design colors should RE-SKIN when the user switches the editor theme.",
  "   weave ships 10 themes (dark + light); each publishes the SAME set of semantic color tokens. ANY color",
  "   field accepts a CSS token STRING of the form `var(--token)` — it is stored verbatim and CSS resolves it",
  "   against the active [data-theme], so flipping the theme recolors the whole deck with NO re-edit. Use a",
  "   token for STRUCTURAL color; reach for a literal hex ONLY where the hue ITSELF is the meaning.",
  "",
  "   STRUCTURAL color → use a token (the default for backgrounds, text, emphasis, panels):",
  ...
  "     • gradient     — keep it theme-reactive by putting tokens in the STOPS, e.g. units:[{ kind:",
  "                      'decoration.fill', attrs:{ type:'linear-gradient', angle:135, stops:[{offset:0,",
  "                      color:'var(--accent)'},{offset:1,color:'var(--accent-strong)'}] } }].",
  "",
  "   LITERAL hex/rgb → ONLY when the hue is fixed meaning, not style: brand / logo colors · data values bound",
  "   to a specific color · photographic content · universal status (success-green / warning-amber / danger-red).",
  "",
  "   Tokens work in EVERY color field — text attrs.color, decoration.fill / .stroke PaintSpec `color`, and the",
