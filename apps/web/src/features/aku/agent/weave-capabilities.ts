...
export const WEAVE_TASK_PRIMER = [
  ...
  "- TABLES / matrices → auto-grid with explicit tracks, NEVER nested flex rows. TEXT is AUTO-HEIGHT: the frame sets its width, the content sets its height — do NOT stretch/grow text to fill a cell or pin a height; a short title keeps its natural height and the frame's align/gap absorbs spare room. Size equal regions with the FRAMES (tracks/grow), not the leaf text.",
  "- ONE point per slide, SHOWN not explained: cut explanatory prose to the minimum (a slide is not a paragraph), make the single key message or number the visual HERO (large, bold, accent color) and keep support quiet/smaller. CHOOSE A FONT SIZE THAT FITS — content is too big if it crowds or overflows its frame; size DOWN and trim words until it sits inside with margin to spare.",
...
export const WEAVE_DOMAIN_KNOWLEDGE = [
  ...
  "",
  "3) TEXT IS AUTO-HEIGHT — THE LAYOUT SETS WIDTH, THE CONTENT SETS HEIGHT. Text is a CHILD of an auto-flex /",
  "   auto-grid frame (containerId = a layout frame): the frame sets the text's WIDTH (cell / cross axis) and",
  "   position; the text box AUTO-FITS its HEIGHT to the wrapped content. Do NOT grow or vertically stretch a text",
  "   box to fill empty space and do NOT pin a fixed height — a short title keeps its natural content height;",
  "   absorb spare room with the frame's justify/align/gap/padding, not by inflating the box. For equal-size",
