  ...
  itemKinds: [
    ...
    },
    {
      ...
      description: [
        ...
        "SIZING: give a PX TARGET — attrs.fontSizeSpec { kind:'px', value } where value is the ABSOLUTE design-px size you want (off the canvas px in the [디자인] line). weave AUTOMATICALLY grounds that px into a ratio of the text's IMMEDIATE parent frame at command time, so the text is responsive AND you do NOT compute the fraction or guess the parent's px height — weave knows it exactly, at any nesting depth. This is the fix for text that came out too big/too small in nested frames: size in plain px, weave converts. SHARED-FRAME BUDGET — the #1 cause of overflow: when a frame holds MORE THAN ONE item the items SHARE its height, so a text does NOT get the whole frame. Before sizing, budget it: usable height = parent frame px height − padding(top+bottom) − the gaps between children; size each text's px against ITS OWN share of that budget, and the SUM of the children's rendered heights (each ≈ fontSize × lineHeight × wrapped-line-count) must fit the usable height WITH MARGIN (the whole stack + gaps ≤ ~85%). Sizing a text as if it alone fills a frame it actually shares with a title / subtitle / body + gaps is exactly what overflows. Px role targets off the CANVAS height: heading ~6–9% (≈65–97px on a 1080px canvas), subheading ~4%, body ~3% (≈32px). These are the SAME px on a full slide OR deep in a small nested card — weave re-bases each to its own parent, so do NOT shrink the px just because the parent frame is small. The size must FIT: if it would crowd or overflow, size the px DOWN and shorten the copy, never oversize. (You MAY pass { kind:'ratio', value:0..1 } directly if you already have the parent-relative fraction; weave leaves a valid ratio as-is. NEVER put a fraction in the plain fontSize number — a 0..1 there renders as sub-pixel text.)",
...
export const WEAVE_TASK_PRIMER = [
  ...
  "- Frame COORDINATES are 0..1 of the immediate parent frame (NEVER pixels). FONT SIZE is the exception: give an ABSOLUTE px via fontSizeSpec { kind:'px', value } (off the canvas px in the [디자인] line) and weave grounds it to a parent-relative ratio for you — do NOT pre-divide by the parent height, and do NOT shrink the px just because the parent frame is nested/short (weave re-bases per parent).",
...
export const WEAVE_DOMAIN_KNOWLEDGE = [
  ...
  "",
  "2) FONT SIZE — GIVE A PX TARGET; weave makes it responsive for you. Set attrs.fontSizeSpec =",
  "   { kind:'px', value } where value is the ABSOLUTE design-px size you want (read the canvas px off the [디자인]",
  "   line). weave AUTOMATICALLY grounds that px into a ratio of the text's IMMEDIATE parent frame at command time,",
  "   so the text stays responsive AND you do NOT compute any fraction — no dividing by parent heights, no",
  "   per-nesting math, no guessing the parent's px height (weave knows it exactly). This is the #1 fix for text",
  "   that came out far too big or too small in nested frames: you size in plain px, weave does the conversion.",
  "   PICK THE PX BY ROLE off the CANVAS height: heading ~5–7% (≈60–84px on a 1080px canvas), subheading ~4%, body",
  "   ~3% (≈32px). These are the SAME px whether the text sits on a full slide or deep in a small nested card —",
  "   weave re-bases each to its own parent, so do NOT shrink the px just because the parent frame is small.",
  ...
  "   would crowd, size the px DOWN and shorten the copy, never oversize. (You MAY pass { kind:'ratio', value:0..1 }",
  "   directly if you already have the parent-relative fraction, but px is simpler and grounded for you. NEVER put a",
  "   fraction into the plain fontSize number — that renders as sub-pixel, invisible text.)",
  "",
