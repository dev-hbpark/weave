...
export const WEAVE_COMMAND_SCHEMAS: Readonly<Record<string, AgentCommandSpec>> = {
  ...
  "weave.item.add": {
    ...
    inputSchema: obj(
      ...
      ["kind"],
      "ADD a SINGLE new item (frame / text / image / video / shape / line / qr / embed) into the design or a container frame. Pass containerId for a parent frame (omit → design root), frame for the 0..1 box, attrsOverride for per-kind content/style, and units to style it (fill/shadow/…) in the SAME call. For a chart use weave.chart.add instead. To create a WHOLE slide/section — a frame with several (possibly nested) items — DO NOT chain many weave.item.add calls; use weave.subtree.add to build the entire tree in ONE call (far fewer turns). Use weave.item.add for a one-off item. CHECK THE TARGET CONTAINER'S LAYOUT FIRST (read it from the snapshot): if the container is ABSOLUTE (no auto-layout) you MUST pass a frame with width>0 AND height>0 — an absolute parent does NOT auto-position its children, so a missing/zero frame lands the item at zero size = invisible & uneditable. If the container has an AUTO-LAYOUT (flex/grid), OMIT the frame — the layout positions and sizes the child; do not fight it with an absolute frame. Do not assume a container is grid; verify.",
    ),
  },
