...
export function makeRoundGroupingEditor(
  ...
  const close = (): void => {
    ...
    if (open) {
      editor.endBatch();
      open = false;
      // DR-053 Stage 3 — the render-timing auto-fit was removed (the layout engine
      // owns sizing). Agent-generated text settles via the engine layout, not a
      // post-round measure-and-fit pulse. (Content→box sizing is a later
      // engine-fed step.)
    }
  };
