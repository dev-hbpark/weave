...
export const RubberBandLayer = forwardRef<HTMLDivElement, RubberBandLayerProps>(
  function RubberBandLayer(
    ...
    const capability = useMemo(() => defaultInsertableRegistry.get(containerKind), [containerKind]);
    ...
    // WI-040 — clear `vm.rubberBand` when the interaction mode leaves the
    // gesture's own envelope. After release, the slot enters `reviewing`
    // / `previewing` and the popover stays open until the user picks or
    // dismisses. If something else now owns the canvas — context-menu
    // (LayerPicker), pan, frame-manipulating drag, text editing — the
    // popover plus rectangle become orphaned chrome competing for the
    // user's pointer. Hard-clear in those cases. We only touch the slot
    // when it belongs to *this* host so co-existing layers in other
    // containers don't trample each other.
    ...
    useEffect(() => {
      ...
      if (
        peekActive ||
        interactionMode === "context-menu" ||
        interactionMode === "panning" ||
        interactionMode === "hand" ||
        interactionMode === "frame-manipulating" ||
        interactionMode === "text-editing"
      ) {
        const slot = vm.rubberBand.get();
        if (slot !== null && slot.hostId === containerId) {
    ...
    }, [interactionMode, peekActive, vm, containerId]);
    ...
    const rb = useMemo(() => {
      ...
      return {
        ...
        previewKind:
          rbSlot.phase === "reviewing" || rbSlot.phase === "previewing" ? rbSlot.previewKind : null,
      };
    }, [rbSlot]);
    ...
    const visualState =
      rb.state === "drawing" ? "drawing" : rb.state === "previewing" ? "previewing" : "reviewing";
    ...
    const commitFn = useCallback(() => {
      ...
      if (slot.hostId !== containerId) return;
      const recId =
        (slot.phase === "reviewing" || slot.phase === "previewing" ? slot.previewKind : null) ??
    ...
    }, [rb.state === "reviewing" || rb.state === "previewing", releasePoint]);
      ...
      >
        ...
        {viewportRect !== null
          ? (() => {
              ...
              const chipDims =
                rb.rect !== null ? { width: rb.rect.width, height: rb.rect.height } : undefined;
              const visual = !hasDragArea ? null : rb.state === "drawing" ? (
                <RubberBand
                  rect={rectV}
                  state="drawing"
        ...
        {popoverOpen && rb.rect !== null ? (
          ...
          >
            {popoverVirtualRef !== null ? <PopoverAnchor virtualRef={popoverVirtualRef} /> : null}
            <PopoverContent
              side={popoverSide}
              align={popoverAlign}
              sideOffset={12}
