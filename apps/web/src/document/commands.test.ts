...
describe("weave.subtree.add — nested subtree creation (WI-141 / DR-096)", () => {
  function subtreeCmd() {
    const c = buildWeaveCommands(spyTargets()).find((x) => x.name === "weave.subtree.add");
    if (c === undefined) throw new Error("command not found");
    return c;
  }
  const creates = (res: ReturnType<ReturnType<typeof subtreeCmd>["run"]>) => {
    if (!res.ok) throw new Error("subtree.add failed");
    return res.patches.filter((p) => p.type === "item.create") as unknown as Array<{
      parentId: unknown;
      item: { id: unknown };
    }>;
  };

  it("creates the whole tree in one transaction; children parented to their node", () => {
    const ctx = makeCtx();
    const rootId = String(ctx.document.root.id);
    const res = subtreeCmd().run(ctx, {
      node: {
        kind: "frame",
        frame: { x: 0, y: 0, width: 1, height: 1, rotation: 0 },
        children: [
          { kind: "text", attrsOverride: { text: "Title" } },
          { kind: "text", attrsOverride: { text: "Body" } },
        ],
      },
    });
    if (!res.ok) throw new Error("unexpected fail");
    const c = creates(res);
    // root frame + 2 text children = 3 item.create patches.
    expect(c).toHaveLength(3);
    // root frame is parented to the design root; its id is the command's return value.
    expect(String(c[0]?.parentId)).toBe(rootId);
    expect(String(c[0]?.item.id)).toBe(res.value);
    // both children are parented to the NEW frame's id (not the design root).
    const frameId = String(c[0]?.item.id);
    expect(String(c[1]?.parentId)).toBe(frameId);
        ...
        children: [
          {
            ...
            children: [{ kind: "text", attrsOverride: { text: "deep" } }],
          },
        ],
      },
    });
    const c = creates(res);
    expect(c).toHaveLength(3); // outer frame, inner frame, text
    const outer = String(c[0]?.item.id);
    const inner = String(c[1]?.item.id);
    expect(String(c[1]?.parentId)).toBe(outer); // inner frame inside outer
    expect(String(c[2]?.parentId)).toBe(inner); // text inside inner frame
  });

  it("is atomic: an invalid node fails the whole call with no patches", () => {
    const res = subtreeCmd().run(makeCtx(), {
      node: {
        kind: "frame",
        frame: { x: 0, y: 0, width: 1, height: 1, rotation: 0 },
        // biome-ignore lint/suspicious/noExplicitAny: testing an invalid kind
        children: [{ kind: "not-a-real-kind" as any }],
      },
    });
    expect(res.ok).toBe(false);
  });

  it("rejects a missing node spec", () => {
    // biome-ignore lint/suspicious/noExplicitAny: testing invalid input
    const res = subtreeCmd().run(makeCtx(), {} as any);
    expect(res.ok).toBe(false);
  });
});
