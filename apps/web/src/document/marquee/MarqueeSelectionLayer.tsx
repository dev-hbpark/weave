...
export interface MarqueeSelectionLayerProps {
  ...
  readonly onSelectIntent: (intent: MarqueeIntent, ids: ReadonlyArray<string>) => void;
...
export const MarqueeSelectionLayer = forwardRef<HTMLDivElement, MarqueeSelectionLayerProps>(
  function MarqueeSelectionLayer(
    ...
    // WI-040 — affordance gate. Marquee selection is a frame-selecting
    // gesture and must stand down whenever another canvas owner is
    // active: peek inspector, context-menu (LayerPicker), hand/pan,
    // rubber-band, frame-manipulating drag, text editing. Read via ref
    // so the capture-phase listener (registered once) consults the
    ...
    useEffect(() => {
      ...
      function finish(e: PointerEvent | null): void {
        ...
        const local = e !== null ? clientToLocalRef.current(e.clientX, e.clientY) : d.start;
