...
- **드래그**: 빈 박스 그려진 영역에 Auto-height 모드로 생성. width = 드래그 width, height = 1 line.
- **더블클릭 (빈 공간)**: 클릭 지점에 Auto-width 모드로 생성. 즉시 편집 진입.
- **추가 메뉴 "T 텍스트"** (현재 동작 유지): 기본 위치/크기로 Auto-height 모드 생성.
