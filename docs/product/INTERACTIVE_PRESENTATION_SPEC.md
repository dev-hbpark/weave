...
| Field | Value |
|---|---|
| Status | **Living spec** (Phase 9~12 의 paradigm drift 정리, 향후 build 의 기준) |
| Owner | hbpark |
| Last update | 2026-05-23 |
| Triggers update | paradigm 결정 / 새 인터랙션 추가 / 도메인 추가 |
| Source WIs | WI-001 (kickoff), WI-009 (interactive presentation PoC), WI-013 (agocraft swap) |
...
|---|---|---|---|
| **root view** | 기본 / Esc | design 전체 fit | root.children |
| **selected frame** | frame 의 outline click | root view 유지 | 그 frame 의 children (`selectedFrameId ?? root`) |
| **entered frame** | frame 더블클릭 / ContextMenu "Enter frame" / SelectionLayer "Enter" 버튼 (future) | 그 frame 의 absolute frame center, scale = 1/max(absW, absH) | 그 frame 의 children |

규칙:
- entered 우선 ≫ selected ≫ root
- Esc / breadcrumb click → 한 단계 위 zoom out
- frame 의 hierarchy 깊은 곳이 entered 일 수 있음 (drill 깊이 무제한)

### 4.2 Add 인터랙션 (frame 생성)
...
### 4.5 ContextMenu (frame 의 right-click)

- **Enter frame** — drill in (zoom 진입)
...
┌─────────────────────────────────────────────────────────────┐
│  Header                                                       │
│   [weave] / [design title] · [entered frame title]            │
...
| Zone | 역할 | 레퍼런스 |
|---|---|---|
| **Header — left** | weave logo + breadcrumb (design title / entered frame) | Prezi 의 좌상단 path indicator |
...
| **ContextMenu** | right-click on frame → Enter / Delete / Duplicate / Move | Figma + Genially 양쪽 표준 |

향후 추가 zone (v1):
- **Right panel (Properties)** — selected frame 의 attrs / interactions 편집. Genially 의 interactive trigger panel 영감.
- **Left rail (Mini-nav)** — design tree (frame hierarchy) sticky thumbnails. drill 진입 시에도 visible.
...
DesignPage — root view
   ...
   ├─→ Frame click (outline) → selected → 8 handle manipulation
   ├─→ Frame right-click → Enter / Delete / Duplicate (future)
   ├─→ Frame double-click → Enter frame (zoom in)
   │     │
   │     ▼ entered view
   │     ├─→ 다시 Add → 그 frame 의 child 로
   │     ├─→ child frame manipulation
   │     ├─→ child frame double-click → 더 깊이 drill (무제한)
   │     └─→ Esc / breadcrumb segment → 한 단계 zoom out
   │
...
| **double-click on frame** | frame chrome | Enter (drill-in zoom) |
| **right-click on frame** | frame chrome | ContextMenu (Enter / Delete / future Duplicate / Move) |
...
| **Esc** | (a) 편집: deselect / exit entered frame / cancel inline edit (b) present: → 편집 모드로 | 양쪽 |
...
| **Enter** (frame selected 시, future) | Enter frame (drill-in) | 편집 모드 |
...
| **drill-in zoom transition** (cubic-bezier spring) | ✓ | Prezi 의 시그니처 — 단 frame center auto-fit (Prezi 는 수동 path) |
...
- [x] drill-in zoom + breadcrumb + Esc / segment click exit (Phase 12c)
