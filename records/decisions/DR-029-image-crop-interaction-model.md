...
## D11 — 회전 시 팬이 회전 전으로 고정되던 문제(회전 피벗 수정) (2026-06-02)

**증상(운영자 보고):** 크롭 후 회전하면 "크롭 영역으로 이동(팬)할 수 있는 이미지 공간이 회전
전으로 고정"됨. 재현(40° 회전 + 팬 극단): 윈도가 이미지 중앙(teal)에 갇혀 가장자리에 도달
불가, 팬이 콘텐츠를 제대로 평행이동시키지 못함.

**근본 원인:** 렌더(`rotationTransform`)가 rotate+cover-zoom을 **소스 중심**(wrapper center =
source 0.5,0.5 매핑) 기준으로 적용. 팬으로 윈도를 옮기면 회전 피벗이 프레임에서 멀어져 콘텐츠가
**평행이동이 아니라 공전**하고, cover-zoom도 윈도가 중앙일 때만 커버 보장(팬 극단에서 갭/오작동).

**오답 시도 & 폐기:** 팬 클램프를 회전/zoom-aware로 확장해 윈도를 [0,1] 밖까지 허용 → cropRatio는
소스 영역이라 `weave.image.setCrop`의 [0,1] 검증이 커밋을 거부(x 변화 없음). 윈도는 반드시 소스
[0,1] 안. 되돌림.

**해결:** 피벗을 **윈도 중심**(= 화면상 프레임 박스 중심)으로 변경. `transformOrigin`을
`"center"` → `${(x+w/2)*100}% ${(y+h/2)*100}%`(img 박스 = 소스 [0,1] 기준 윈도 중심).
이러면 (1) 어느 팬 위치에서도 회전이 제자리에서 돌고, (2) cover-zoom이 항상 프레임 중심 기준이라
팬 극단에서도 갭 없음, (3) 팬은 [0,1-w] 그대로(데이터 모델 유지)이되 회전 콘텐츠를 올바르게
평행이동. `coverZoom`을 crop-geometry로 이관(공유). 가장자리는 cover-zoom 특성상 여전히 크롭됨
(독립 이미지 스케일 핸들 = P3 backlog).

검증: crop-geometry 단위 11(팬 [0,1] 클램프는 회전 무관/부분 팬 동일/coverZoom), e2e — 회전 후
