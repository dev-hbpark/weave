...
## 9. WI-170 — 기본 판단 규칙: 명시 없으면 새 페이지 (프롬프트 계약 확장)

사용자 지시: 기존 아이템/프레임/슬라이드의 수정·변경 요청이 명시되지 않으면
기본적으로 **새 페이지(슬라이드) 생성 후 그 안에서 디자인 작업**으로 판단.

- 레버 = `pagePromptFragment` 활성-페이지 분기 (per-submit 주입되는 pageLine).
  "요청이 기존 콘텐츠를 겨냥하는가"는 자연어 해석이라 스키마/매퍼/게이트로
  내릴 수 없는 **판단 규칙** — DR-115 §2의 "스키마가 모델, 프롬프트는 라이브
  앵커 + 스키마가 표현 못 하는 규칙" 원칙의 두 번째 적용 사례.
- 빈-활성-페이지 엣지는 모델의 스냅샷 확인에 위임("비어 있으면 그대로 사용,
  콘텐츠 있으면 weave.page.add 후 작업"). promptFragment는 순수 함수 유지.
- FREE_AGENT_SURFACE(mixed/canvas-board)는 제외 — 자유 캔버스 의미론.
- WI-169의 도구 채널(page.add → FULL_FRAME → 동기 활성화)이 선행 조건:
  모델이 page.add를 고르기만 하면 레일 패리티가 나머지를 보장한다.
