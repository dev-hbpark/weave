...
## 폰트 px 고정 전환 ([DR-101](../decisions/DR-101-px-fixed-font.md), 2026-06-08)

사용자 보고(프레임 리사이즈 시 텍스트가 점점 작아짐) 분석 결과 **ratio 폰트가 프레임 높이에 결합**된 게
원인(DR-091). px 고정으로 전환:
- `groundAgentFontSize`(px→ratio) **제거**(데코미션) → 폰트 리터럴 px 유지, 리사이즈/중첩 재배율 없음.
- **WI-146 B(`a.frame.height` 재보정 트리거) 되돌림** — 리사이즈 중 간섭 유발. 에이전트 정착은 B-2(라운드
  종료 펄스)가 담당하므로 불필요.
- 가이드(capabilities/command-schemas)를 px-고정으로 갱신(본문 최소치 유지, ratio 비권장).
- DR-091 SUPERSEDED 표기. 검증: 단위 233, layout e2e 2/2, typecheck·biome 클린.

## 진행 로그

- 레코드 3종 → 순수 코어(테스트) → 증분1+2 핸들 → 증분3 병합. 증분별 커밋.
- 후속: base-case 정상(e2e) → 나중 height 쓰기 재발화 없음(B) → 라운드 종료 일괄 재보정(B-2) →
  리사이즈 축소 분석 → **px 고정 전환(DR-101, B 되돌림)**.
