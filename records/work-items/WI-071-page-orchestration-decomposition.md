...
- 2026-06-01 — **Phase 2 뷰 분리 완료 (DesignDialogs · renderFrameMenu lift · PeekCaptureLayer · SelectionToolbarOverlay).** (1) `view/DesignDialogs.tsx` — 모달 5개(MediaSrc/PasteSpecial/Conflict/SlidePreset/CommandPalette)를 ISP-narrow props로; media-confirm/slide-preset 변이 로직은 오케스트레이터 핸들러(`handleMediaConfirm`/`handlePickPreset`/`mediaInitialSrc`)로 lift. (2) `renderFrameMenu`(~95줄 reparent/convert/zorder 메뉴 빌더)를 FrameStage slot 인라인 → named const로 lift(`FrameMenuContext` 타입). (3) `view/PeekCaptureLayer.tsx` — peek capture div + 핸들러 + PeekOverlay + PointStackInspector(응집형, ~12 peek props, isActive 자체 게이트). (4) `view/SelectionToolbarOverlay.tsx` — SelectionChromeGate + body-portal ContextualToolbar + deep-walk selectedItems(~5 props, useSelectionChromeVisible 자체 게이트). **37-prop 단일 DesignCanvas forwarding shell 대신 응집형 2개로 분해** — `<main>`/FrameStage/renderHoverOverlay/PresenceCursors는 오케스트레이터 잔류(FrameStage는 이미 컴포넌트). **Decommission sweep**: 로컬 SelectionChromeGate def + 미사용 import(PeekOverlay·PointStackInspector·ContextualToolbar·useSelectionChromeVisible) 제거. 검증(매 단계): typecheck 0, 신규 4파일 biome clean, DesignPage 0err/21warn, vitest 328, e2e 누계 add-menu·clipboard·peek-mode·figma-quickaction·reparent·shape-line-convert·frame-menu **다수 green**. 커밋 6개(`d77446d`·`104420c`·`086f658` 등). **DesignPage.tsx 4574→3186줄(−1388, −30%).** 뷰 컴포넌트: DesignHeader·DesignDialogs·PeekCaptureLayer·SelectionToolbarOverlay.
- 2026-06-01 — **Phase 3 FrameStage 분해 (순수 모듈 + NestedFrame + viewport-culling).** (1) `frame-stage/camera-math.ts`(`nextPanForZoom` pan-anchor 대수) + `frame-stage/luminance.ts`(`perceivedLuminance` WCAG probe) — 순수 추출(AUDIT-006 F-1 MED 해소). (2) `frame-stage/NestedFrame.tsx` — 재귀 프레임 렌더러 ~677줄을 별 파일로(props는 `FrameStageProps[...]` 인덱스 타입, type-only import로 런타임 순환 없음). import 21개 path-조정 + prune(FrameStage 19 / NestedFrame 2 미사용 제거). (3) `frame-stage/use-viewport-culling.ts` — WI-058/DR-021 IntersectionObserver cull registry(입력 infiniteCanvas+outerRef, 반환 registry). 검증(매 단계): typecheck 0, 신규 파일 biome clean, FrameStage 0err, vitest 328, e2e shape-poly·line-endpoint·text-item·add-menu·peek-mode·zorder-restore·handle-fsm-resize **51+ passed**. 커밋 3개(`afd7a20`·`4149fcf` 등). **FrameStage.tsx 2435→1545줄(−890, −37%).** 남음(최고위험·선택): FrameStage 내부 `use-camera`(MotionValue 카메라·휠·줌, 렌더 transform 직결) + `use-frame-gesture-router`(GestureRouter 호스트 + FrameAccess ref) — 줌/팬 e2e 커버리지 제한적이라 별도 신중 패스 권고.
- 2026-06-01 — **FrameStage 내부 camera/gesture-router 추출 분석 → STOP 결정(net-negative).** 두 클러스터를 정밀 검토한 결론: 분리 불가능한 코어 엔진. ① `frameAccess`(625–939, ~315줄 memo)가 흩어진 로컬 ref 6개(`docRef`/`onSelectRef`/`onUpdateItemRef`/`onCommitFrameRef`/`moveSelectionSessionRef`/`panActiveRef`)에 의존. ② gesture register 효과(959–1044)가 `frameAccess`+`router`+`designAdaptedCapability`+**카메라 소유 `clientToDesignLocal`/`panActiveRef`/`outerRef`** 를 동시 사용 → gesture↔camera 교차 결합. ③ camera는 `pan` state→MotionValue(`planeTxMV`/`totalScaleMV`)→**렌더 transform 직결**(12+ 반환 필요). 추출 시 ~11입력+ref6 재배선 600줄 gesture 훅 / 12+반환 camera 훅이 되며 FrameStage에 완전 재결합 + 유일하게 flaky한 `frame-handles` e2e가 안전망. **위험(코어 편집 resize/rotate/drag/zoom) > 가치(파일크기)** → FrameStage 1545줄(카메라+제스처+렌더 코어)을 정지점으로 확정. 향후 필요 시 별도 집중 세션 + 카메라/제스처 e2e 보강 선행 권고.
- **누계: DesignPage 4574→3186(−30%), FrameStage 2435→1545(−37%). 신규 모듈 18개(훅 9 + 뷰 4 + frame-stage 4 + line-seeds). 전 단계 typecheck/vitest/biome/e2e 게이트 통과. WI-071 실질 완료.**
- 2026-06-01 — **Phase 2 뷰 분리 착수: `DesignHeader` 추출 + 공유 `line-seeds` 모듈.** (1) `pages/design/line-seeds.ts` 신규 — `LINE_*` 시드 4종 + `LineSeed` 타입을 헤더/FrameAddSubmenu 공유 모듈로 분리. (2) `pages/design/view/DesignHeader.tsx` 신규 — 인라인 `<header>`(453줄, createPortal→document.body 포함)를 **ISP-narrow props 15개**(데이터 + intent 콜백, peek/vm/editor 객체 미노출)로 추출. `SAVE_*` 룩업테이블(헤더 전용)을 함께 이동. CommandIconButton(undo/redo)은 provider 트리 컨텍스트로 동작. **Decommission sweep**: DesignPage에서 헤더-전용 미사용 import 15개(Button·ColorPicker·ThemePicker·IconCloud*·IconCursor·IconHand·IconPlay·IconQr·IconUndo·IconRedo·IconButton·Link·DropdownMenuLabel) + `SAVE_*` 상수 제거. 검증: typecheck 0, 신규 파일 biome clean(DesignHeader 1 advisory=`role=group` a11y, 원본 헤더에 있던 것), DesignPage **0err/21warn(베이스라인 1err/22warn보다 개선)**, vitest 328 green, **e2e add-menu+peek-mode+figma-drag-to-add 14 passed**(1 flaky-but-passed). **DesignPage.tsx 3877→3414줄.** 누계 4574→3414(−1160). 남음: Phase 2 DesignCanvas/DesignDialogs + 오케스트레이터 수렴, Phase 3 FrameStage.
