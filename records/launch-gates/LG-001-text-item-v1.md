...
| ~~R4 e2e 4 deferred specs~~ ✅ **Landed 2026-05-28** (`apps/web/e2e/text-r4-deferred.spec.ts`, safety-net suite): (1) Korean Hangul commit → textRuns ✅ PASS via `keyboard.insertText` (the path real IMEs flush to after `compositionend`). (2) Cmd+B/I/U range style — `test.fixme` × 3 with documented Playwright + Lexical interaction gotcha (Mod+B/I/U keyboard.press does not fire Lexical FORMAT_TEXT_COMMAND because no `beforeinput` is synthesised; rich-text per-range mouse-trigger path is exercised by the PropertiesPanel controls). (3) Mount/unmount/remount cycle ✅ PASS — Lexical state does not leak across edit-mode entries. (4) 2-actor concurrent — `test.skip` per WI-028 sync paused; re-enable condition pinned. Net: 2 active PASS + 4 documented skips. | ~~Engineering + QA~~ | ~~hbpark~~ | ~~launch -1주~~ |
