...
### `Panel position="floating"` — overlay 표면으로 수정 (DS 프리미티브)

증상: Aku 패널(유일한 `floating` 소비처)이 다크 Aurora 앱에서 **흰색으로 워시아웃** — 헤더 타이틀/버튼이
white-on-white로 안 보임. 원인: `floating`이 baseClass의 `--surface-1`(= 6% 흰색 *테마 글래스*, 앱
자체 다크 bg 위 inline chrome용)을 그대로 써서, 임의 색의 **디자인 캔버스 위에 떠 있을 때** 캔버스에
녹아버림(실측 `panelBg = rgba(255,255,255,0.06)`).

수정: `positionClass.floating`이 tokens.css의 **floating-chrome 토큰**(`--surface-overlay` 불투명
다크 0.94 / `--surface-overlay-border` / `--text-overlay` / `--shadow-overlay`)을 쓰도록 변경
(tokens.css:280 주석의 지침 그대로 — "floating chrome은 테마 스킨이 아닌 단일 제품 레이어"). `cn`이
`twMerge`라 positionClass가 baseClass 표면 유틸을 덮어씀. **docked 패널은 무영향**(여전히 `--surface-1`).
실측 수정 후 `panelBg = rgba(15,23,42,0.94)`, 코치마크/팝오버와 동일한 다크 글래스로 정상화.
...
- `packages/design-system/src/components/Panel.tsx` — `floating` 표면을 overlay 토큰으로 수정
